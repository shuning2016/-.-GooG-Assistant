...
Flow:
  ...
  2. Fetch Gmail / Calendar / Drive data via Google APIs.
  3. Call Claude to generate the briefing markdown.
  4. Store markdown in Upstash Redis (TTL 7 days).
  5. Send HTML email to Shuning with a "View in browser" link.
...
class handler(BaseHTTPRequestHandler):
    ...
    def _handle(self):
        # Verify Vercel cron secret (Vercel injects Authorization: Bearer <CRON_SECRET>)
        ...
        try:
            ...
            window = f"{since.strftime('%Y-%m-%d %H:%M SGT')} → {now_sgt.strftime('%Y-%m-%d %H:%M SGT')}"
            ...
            logging.exception("Daily brief failed")
