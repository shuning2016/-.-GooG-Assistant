...
- Default lookback: always the past 24 hours from time of run (rolling window, not since last briefing)1
...
2. Read `.claude/state/daily-brief.json` only to record the previous run for the delta comparison in Step 8.
...
|--|--------|--------|-----|--------|
| 🔴 | [action text] | Email / SeaTalk: [name] | overdue / today | Chase now |
...
**Adding new items:** When you find a new action item for Shuning in a key-domain email thread or SeaTalk message, append it to `.claude/state/open-action-items.json` before producing the briefing. Schema:
```json
{
